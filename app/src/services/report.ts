...
type ReportCounts = {
  [key in ReportType]: {
    [severity: string]: SeverityCounts;
  };
};
...
function parseDate(dateStr: string): Date | null {
  ...
  // sto codice è figo pero voglio qualcosa che manda sta risposta se la data è futura return res.build("BadRequest",`cannot report for future days`);
}
...
class ReportService {
  ...
  async createReport(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      if (date.length < 10 || isNaN(parseDate(date))) {
        return res.build(
          'BadRequest',
          `Invalid date value. Example of a valid value: "2023-07-21T15:00:00Z", "2023-07-21", "2023-21-07", "2023/07/21", "21-07-2023"`
        );
      }
      if (parseDate(date) > new Date()) {
        return res.build('BadRequest', `Cannot create report for future days.`);
      }
      if (
        !position ||
        position.type !== 'Point' ||
        ...
        typeof position.coordinates[0] !== 'number' ||
        typeof position.coordinates[1] !== 'number'
      ) {
      ...
      if (![...Object.values(Severity.Pothole), ...Object.values(Severity.Dip)].includes(severity)) {
        return res.build(
          'BadRequest',
          `Invalid Severity value. Available values: ${reportTypeCheck ? Object.values(Severity.Pothole).join(', ') : Object.values(Severity.Dip).join(', ')}`
        );
      }
  ...
  async deleteReportById(req: Request, res: Response, next: NextFunction) {
    try {
      const id = req.params.id;
      ...
      const result = await reportRepository.deleteReport(report);

      res.build('OK', 'Risultato eliminazione report', result);
    } catch (err) {
      next(ISError('Errore durante il recupero del report.', err));
    }
  }
  ...
  async updateReport(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      ...
      if (parseDate(data.date) > new Date()) {
        return res.build('BadRequest', `Cannot update report for future days.`);
      }
  ...
  async generateInitialStatusCounts(): Promise<ReportCounts> {
    const reports = await reportRepository.getReports();
  ...
  async runDbscan(req: Request, res: Response, next: NextFunction) {
    ...
    if (isNaN(eps) || isNaN(minPts) || !Number.isFinite(eps) || !Number.isInteger(minPts) || eps <= 0 || minPts < 1) {
    ...
    try {
      ...
      const result = await reportRepository.runDbscan({
        data: positions,
        eps,
        minPts,
      });
